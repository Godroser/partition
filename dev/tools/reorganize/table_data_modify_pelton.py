...
def split_table_sql(table_name, replica_columns, partition_keys, replica_partition_keys):
    ...
    output_dir = "/data3/dzh/project/grep/dev/tools/reorganize/data/50pelton"
...
def modify_table_data(table, replica_columns, partition_keys, replica_partition_keys):
    input_dir = "/data3/dzh/CH-data/50ch"
    data_dir = "/data3/dzh/project/grep/dev/tools/reorganize/data/50pelton"
    ...
    if replica_columns:
        ...
        with get_connection(autocommit=False) as connection:
            with connection.cursor() as cur:     
                cur.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = '{}' AND table_name = '{}';".format('50pelton', table))

                if len(cur.fetchall()) > 0:
                    cur.execute('DROP TABLE {}'.format(table))
                    print("Drop table {};".format(table))

                cur.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = '{}' AND table_name = '{}_part1';".format('50pelton', table))
                ...
                cur.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = '{}' AND table_name = '{}_part2';".format('50pelton', table))
                ...
                # 设置副本
                set_replica_sql = f"ALTER TABLE `50pelton`.`{table}_part2` SET TIFLASH REPLICA 1;"
...
if __name__ == "__main__":
    ...
    tables = ['district', 'item', 'new_order', 'orders', 'order_line', 'stock', 'warehouse', 'history', 'nation', 'region', 'supplier']
    # tables = ['customer']
