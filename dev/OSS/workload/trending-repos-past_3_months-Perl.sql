...
WITH /*PLACEHOLDER*/ /*trending-repos-past_3_months-Perl.sql*/ stars AS (
  SELECT
      ...
      SUM(
          GREATEST (
              LEAST (
                  (
                      (
                          TIMESTAMPDIFF(SECOND, DATE_SUB('2022-09-28 08:59:59', INTERVAL 3 MONTH), ge.created_at) / 
                          TIMESTAMPDIFF(SECOND, DATE_SUB('2022-09-28 08:59:59', INTERVAL 3 MONTH), '2022-09-28 08:59:59')
                      ) * (5 - 2)
                  ), 5
              ), 2
          )
  ...
  WHERE
      ...
      type = 'WatchEvent'
      AND (ge.created_at >= DATE_SUB('2022-09-28 08:59:59', INTERVAL 3 MONTH) AND ge.created_at <= '2022-09-28 08:59:59')
  GROUP BY ge.repo_id
...
SELECT /*PLACEHOLDER*/ /*trending-repos-past_3_months-Perl.sql*/ 
    tr.repo_id,
    tr.repo_name,
